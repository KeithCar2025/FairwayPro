...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/geocode", async (req, res) => {
    try {
      ...
      if (!address) {
        return res.status(400).json({ status: "INVALID_REQUEST", error: "Missing address" });
      }

      const key = process.env.GOOGLE_MAPS_API_KEY;
      if (!key) {
        console.error("Missing GOOGLE_MAPS_API_KEY env");
        return res.status(500).json({ status: "SERVER_CONFIG_ERROR", error: "Missing GOOGLE_MAPS_API_KEY" });
      }
      ...
      // Pass through the Google Geocoding API response verbatim
      return res.status(200).json(data);
    } catch (err: any) {
      console.error("Server geocode error:", err);
      return res.status(500).json({ status: "ERROR", error: err?.message || "Geocode proxy failed" });
    }
  });
